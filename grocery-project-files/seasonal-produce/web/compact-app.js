...
(function () {
  ...
  const GROUP_COLORS = {
    ...
    'Pome Fruit': 'var(--color-pome-fruit)',
    ...
    'Nut': 'var(--color-nut)',
    'Edible Flower': 'var(--color-flower)',
    'Grain': 'var(--color-grain)',
    'Nightshade': 'var(--color-pepper)',
  ...
  // --- Data Loading ---
  function loadData() {
    // Use fetch for HTTP, fallback to XHR for file:// protocol
    var dataPromise;
  ...
  function getItemStatus(slug, month) {
    ...
    // Check if coming into season next month
  ...
  // --- Render: Month Scrubber ---
  function renderScrubber() {
    var container = document.getElementById('month-scrubber');
    container.innerHTML = '';

    MONTH_ABBRS.forEach(function (abbr, i) {
      var btn = document.createElement('button');
      btn.className = 'month-btn' + (i + 1 === state.selectedMonth ? ' active' : '');
  ...
  function renderTimeline() {
    ...
    if (items.length === 0) {
      body.innerHTML = '<div style="padding: 40px; text-align: center; color: var(--text-tertiary);">No produce matches your filters.</div>';
      return;
    }

    // Now line in body — width must match --timeline-width so % aligns with bars
    ...
    items.forEach(function (item, idx) {
      ...
      nameCell.textContent = item.produce.name;
  ...
  function renderBars(container, seasons, color, sourceType, isDual) {
    // Merge all season months from these entries
    var allMonths = {};
    var peakMonths = {};
    ...
    segments.forEach(function (seg) {
      ...
      if (isDual) bar.classList.add(sourceType === 'import' ? 'bar-import' : 'bar-local');
      ...
      // Render peak/available sub-segments within this bar
  ...
  function buildSegments(allMonths, peakMonths) {
    ...
    // Split into contiguous segments (handling wrap)
    var segments = [];
    ...
    // Check for wrap-around: if first segment starts at 1 and last ends at 12,
    // they might be one continuous season — but we still render them as two visual bars
    // since the timeline is linear Jan-Dec
  ...
  function renderGrid() {
    ...
    if (items.length === 0) {
      container.innerHTML = '<div style="padding: 40px; text-align: center; color: var(--text-tertiary);">No produce matches your filters.</div>';
      return;
    }
  ...
  function renderDetail() {
    ...
    // Notes (from first season entry that has them)
    var notes = '';
    var storageTip = '';
    for (var i = 0; i < seasons.length; i++) {
    ...
    // Deduplicate by region + source combo
    var seen = {};
    seasons.forEach(function (s) {
